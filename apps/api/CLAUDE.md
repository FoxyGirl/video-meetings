...
`AuthModule` (`src/auth/`) implements registration and login, both driven by the e2e tests in `test/auth.e2e-spec.ts` (written first, per this feature's TDD approach). It follows the CQRS pattern via `@nestjs/cqrs` rather than a conventional service layer — there is no `AuthService`.
...
- `RegisterHandler` (`@CommandHandler`) does the actual work for register: checks for an existing email (`409 ConflictException` if taken), hashes the password with `bcrypt` (10 salt rounds — never store or compare plaintext), creates the `User` row, publishes a `UserRegisteredEvent` via `EventBus`, and signs the JWT.
- `LoginHandler` (`@QueryHandler`) does the actual work for login: looks the user up (never creates one), verifies the password hash, and signs the JWT. `401 UnauthorizedException` for either an unknown email or a wrong password — deliberately not distinguished, to avoid leaking which emails are registered.
- `UserRegisteredHandler` (`@EventsHandler`, `src/auth/events/`) reacts to `UserRegisteredEvent` — currently just an audit log line. This is the extension point for anything that should happen after a registration (welcome email, analytics, etc.) without bloating `RegisterHandler`.
- All three handlers (plus `CqrsModule` itself) are registered as `providers` in `AuthModule`; there's no separate "module per handler" — keep new command/query/event handlers there too as the module grows.
