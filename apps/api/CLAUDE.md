...
- `prisma/schema.prisma` defines the models: `User` (`id`, `email` (unique), `password`, `createdAt`) and `Meeting` (`id`, `title`, `date`, `participants` (string array), `organizerId` FK to `User` with `onDelete: Cascade`, `createdAt`, plus nullable file metadata columns `fileOriginalName`, `filePath`, `fileMimeType`, `fileSize`, `fileUploadedAt` — see "Meetings" below). `prisma.config.ts` (repo convention as of Prisma 7 — connection URL no longer lives in the schema file) points Migrate at `DATABASE_URL`.
...
- `DELETE /meetings/:id/file` (`DeleteMeetingFileCommand`/`DeleteMeetingFileHandler`, `src/meetings/commands/`) is organizer-scoped using the same `findFirst`-by-`id`-and-`organizerId` shape as upload (`404`, not `403`, for a non-organizer or nonexistent meeting); `404` again if the meeting has no file. Crash-safe ordering matches the upload replace path: the row's five file columns are cleared first, then the file is unlinked from disk — a crash between the two leaves at worst an orphaned file, never a row pointing at a deleted one.
