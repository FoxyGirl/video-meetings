...
- `register.spec.ts` registers its own throwaway users (unique, timestamped emails) through the UI/API and deletes each one directly from Postgres in a `test.afterEach` (via the `pg` devDependency, `DATABASE_URL` env var, default `postgresql://postgres:postgres@localhost:5432/video_meetings` — same default as `apps/api`'s `.env.example`) so repeated runs don't pile up rows in the dev `User` table. There's no delete-user API endpoint to call instead — this mirrors how `apps/api`'s own e2e suite cleans up the `User` table by talking to Postgres directly rather than through HTTP. `Meeting` rows cascade-delete with their organizer, so no separate cleanup is needed there.
