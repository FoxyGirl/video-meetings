...
- `getAvatarBlob()` fetches `GET /users/me/avatar` with `responseType: 'blob'` — same Bearer-authenticated-blob reasoning as `downloadMeetingFile` below, but the caller keeps the resulting object URL alive for as long as it's rendered (persistent `<img>`) rather than revoking it immediately after a one-off download click.
